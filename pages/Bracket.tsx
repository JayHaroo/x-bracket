...
export default function Bracket({ players, tournamentName, tournamentType }) {
  ...
  const generateBracket = () => {
    ...
    for (let i = 0; i < shuffledPlayers.length; i += 2) {
      if (i + 1 < shuffledPlayers.length) {
        initialRound.push([shuffledPlayers[i], shuffledPlayers[i + 1]]);
      } else {
        // Wildcard (automatic advance)
        initialRound.push([shuffledPlayers[i], { name: 'Wildcard (Bye)', score: 0 }]);
      }
    }
  ...
  const playMatch = (winner) => {
    ...
    if (updatedRounds[nextRoundIndex].length % 2 === 0) {
      ...
      updatedRounds[nextRoundIndex][updatedRounds[nextRoundIndex].length - 1].push(winner);
    }
  ...
  return (
    <ScrollView className="flex-1 bg-[#121212] px-4 py-6">
      <Text className="text-white text-3xl font-bold mb-4 text-center">{tournamentName || 'Tournament Bracket'}</Text>

      {rounds.map((round, roundIndex) => (
        <View key={roundIndex} className="mb-6">
          <Text className="text-white text-xl font-semibold mb-2">Round {roundIndex + 1}</Text>
      ...
      <View className="mt-6">
        {getCurrentMatch() && (
          <>
            ...
            <Text className="text-white mb-2">Select Winner:</Text>
            {getCurrentMatch().map((player, idx) => (
              <Pressable
                key={idx}
                className="bg-[#1DB954] rounded-full px-4 py-2 my-1"
        ...
        {!getCurrentMatch() && currentRound === rounds.length - 1 && rounds[rounds.length - 1].length === 1 && rounds[rounds.length - 1][0].length === 1 && (
          <Text className="text-white text-center text-2xl mt-10">🏆 Winner: {rounds[rounds.length - 1][0][0].name}</Text>
        )}
      </View>

      <View className="mt-10">
        <Text className="text-white text-lg font-semibold mb-2">Match History</Text>
